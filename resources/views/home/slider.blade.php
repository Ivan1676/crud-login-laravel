...
<div class="mb-16 text-3xl font-extrabold text-gray-900 dark:text-white md:text-5xl lg:text-6xl">
...
<div id="carousel" class="relative w-2/3 mb-32 duration-1000" data-carousel="slide">
    ...
    <div class="relative h-96 md:h-96">
        @foreach ($covers as $index => $cover)
            <div class="carousel-slide" data-carousel-item>
                <img src="{{ $cover }}" alt="{{ $names[$index] }}" class="w-full h-full object-cover">
            </div>
        @endforeach
    </div>
    <!-- Slider indicators -->
    <div class="absolute z-30 flex space-x-3 -translate-x-1/2 bottom-5 left-1/2">
        @foreach ($covers as $index => $_)
    ...
    <!-- Slider controls -->
    <button type="button"
        class="absolute top-1/2 left-4 z-30 flex items-center justify-center h-10 px-4 cursor-pointer group focus:outline-none"
        data-carousel-prev>
        <span
    ...
    <button type="button"
        class="absolute top-1/2 right-4 z-30 flex items-center justify-center h-10 px-4 cursor-pointer group focus:outline-none"
        data-carousel-next>
        <span
            class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
            <svg class="w-4 h-4 text-white dark:text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
